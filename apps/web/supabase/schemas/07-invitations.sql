...
-- OR invitations sent to their email address
 ...
 * @description Get all pending invitations for the current authenticated user's email
 */
...
grant execute on function public.get_user_invitations() to authenticated, service_role;

-- Additional RLS policy to allow invited users to read account details
-- This allows users with pending invitations to see the account name/details
-- when viewing their invitation
create policy accounts_read_for_invited_users on public.accounts for
select
  to authenticated using (
    exists (
